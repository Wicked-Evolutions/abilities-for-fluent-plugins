...
add_action( 'wp_abilities_api_init', function() {
	...
	$reg->read( 'fluent-crm/get-funnel-conversion', array(
		'label'       => 'Get Funnel Conversion',
		'description' => 'Calculate step-by-step conversion rates through a defined sequence of tags and/or events. Shows how many contacts reach each step and the drop-off between steps.',
		'category'    => 'fluent-crm',
		'input_schema' => array(
			'type'       => 'object',
			...
			'properties' => array(
				'steps' => array(
					'type'        => 'array',
					'description' => 'Ordered funnel steps. Each step has type (tag or event) and value (tag title or event_key).',
					'items'       => array(
						'type'       => 'object',
						...
						'properties' => array(
							'type'  => array(
								'type'        => 'string',
								'description' => 'Step type: tag or event',
								'enum'        => array( 'tag', 'event' ),
							),
							'value' => array(
								'type'        => 'string',
								'description' => 'Tag title or event_key string',
							),
						),
					),
				),
		...
		'output_schema' => fluent_abilities_schema_item_output( array(
			...
			'conversion_funnel'=> array( 'type' => 'array', 'items' => array( 'type' => 'object' ) ),
		) ),
		'callback' => function( $input ) {
			global $wpdb;
			$prefix = $wpdb->prefix;
			...
			if ( empty( $steps ) || count( $steps ) < 2 ) {
				return fluent_abilities_error( 'ability_invalid_input', 'At least 2 funnel steps are required.' );
			}
			if ( count( $steps ) > 10 ) {
				return fluent_abilities_error( 'ability_invalid_input', 'Maximum 10 funnel steps allowed.' );
			}
			...
			$funnel_results = array();
			$first_step_count = 0;

			foreach ( $steps as $index => $step ) {
				$step_type = $step['type'];
				$step_value = sanitize_text_field( $step['value'] );

				$count = 0;

				if ( 'tag' === $step_type ) {
					$date_clause = '';
					$params = array( $step_value );

					if ( $date_from ) {
						$date_clause .= ' AND p.created_at >= %s';
					...
					$count = (int) $wpdb->get_var( $wpdb->prepare(
						"SELECT COUNT(DISTINCT p.subscriber_id)
						FROM {$prefix}fc_subscriber_pivot p
						JOIN {$prefix}fc_tags t ON p.object_id = t.id
						WHERE p.object_type LIKE '%%Tag' AND t.title = %s{$date_clause}",
						...$params
					) );
				} elseif ( 'event' === $step_type && $has_events ) {
					$date_clause = '';
					$params = array( $step_value );
				...
				$prev_count = $index > 0 ? $funnel_results[ $index - 1 ]['count'] : $count;

				$funnel_results[] = array(
					'step'     => $step_value . ' (' . $step_type . ')',
					'count'    => $count,
					'rate'     => $first_step_count > 0 ? round( $count / $first_step_count * 100, 1 ) . '%' : '0%',
					'dropoff'  => $index > 0 && $prev_count > 0
						? round( ( 1 - $count / $prev_count ) * 100, 1 ) . '%'
						: '0%',
				);
			}

			$last_count = end( $funnel_results )['count'];

			return array(
				'funnel'              => $funnel_results,
				'overall_conversion'  => $first_step_count > 0 ? round( $last_count / $first_step_count * 100, 1 ) . '%' : '0%',
				'total_at_top'        => $first_step_count,
				'total_at_bottom'     => $last_count,
				'date_range'          => array(
					'from' => $date_from,
					'to'   => $date_to,
				),
